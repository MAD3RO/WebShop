    ...
    public class ShopController : Controller
    {
        ...
        public ActionResult ProductDetails(string name)
        {
            ...
            using (Db db = new Db())
            {
                ...
                if (!db.Products.Any(x => x.Name.Replace(" ", "-").ToLower().Replace(".", "").Replace("/", "").Equals(name)))
                {
                    return RedirectToAction("Index", "Shop");
                }

                // Init productDTO
                dto = db.Products.Where(x => x.Name.Replace(" ", "-").ToLower().Replace(".", "").Replace("/", "") == name).FirstOrDefault();
